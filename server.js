  ...
  try {
    ...
    if (registration.rows.length === 0) {
      return res.status(400).json({
        error: "Vous devez avoir participé à l'événement pour le noter",
      });
    }
    ...
    if (existingRating.rows.length > 0) {
      return res
        .status(400)
        .json({ error: "Vous avez déjà noté cet événement" });
    }

    // Créer la note
    await db.query(
      `INSERT INTO ratings (
        event_id, user_id, overall_rating, ambiance_rating, affluence_rating,
        organisation_rating, quality_rating, value_rating, comment, is_anonymous
    ...
    // Mettre à jour la moyenne de l'événement
...
app.put("/api/ratings/:ratingId", async (req, res) => {
  try {
    ...
    const checkResult = await db.query(checkQuery, [ratingId]);
    ...
    // Supprimé : modification du titre et photos d'événement depuis les avis
    // Ces modifications doivent se faire dans "Mes événements"
...
app.delete("/api/ratings/:ratingId", async (req, res) => {
  try {
    ...
    await db.query(deleteQuery, [ratingId, decoded.userId]);
